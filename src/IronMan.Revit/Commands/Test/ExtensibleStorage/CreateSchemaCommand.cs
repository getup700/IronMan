...
namespace IronMan.Revit.Commands.Test.ExtensibleStorage
{
    ...
    {
        private Guid guid = new Guid("3C4CDD40-30C2-4528-A2BA-0C63881AFC02");
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            UIDocument uiDoc = commandData.Application.ActiveUIDocument;
            Document doc = uiDoc.Document;
            var uiprovider = ServiceLocator.Current.GetInstance<IUIProvider>();

            #region Create extension storage
            Schema schema = SchemaFactory.CreateSchema(new SampleSchemaInfo(uiprovider));
            Schema subSchema = SchemaFactory.CreateSchema(new SubschemaInfo(uiprovider));
            try
            {
                using (SchemaBuilder schemaBuilder = new SchemaBuilder(guid))
                {
                    ...
                    schemaBuilder.AddSimpleField("entity", typeof(RvtEntity)).SetSubSchemaGUID(guid);
            ...
            var value2 = entityGet2.Get<string>("float", DisplayUnitType.DUT_GENERAL);
